...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = 'black') {
  ...
  // Render the text at 4x the original size on the 2D canvas for high resolution
  const scaleFactor = 4;
  const scaledFont = font.replace(/(\d+)px/, (match, size) => {
    return `${parseInt(size, 10) * scaleFactor}px`;
  });

  context.font = scaledFont;
  const metrics = context.measureText(text);
  const textWidth = Math.ceil(metrics.width);
  const textHeight = Math.ceil(getFontSize(scaledFont) * 1.2);
  ...
  // Enable mipmaps and use LINEAR/MIPMAP filters for optimal clarity at all distances/angles
...
export default function CircularGallery({
  ...
  const handlePrev = () => {
    if (appRef.current) {
      appRef.current.scroll.target -= appRef.current.scrollSpeed * 8;
  ...
  return (
    <div className="relative w-full h-full group/gallery">
      ...
      <button
        onClick={handlePrev}
        className="absolute left-6 top-1/2 -translate-y-1/2 z-30 w-12 h-12 rounded-full border border-slate-200/50 bg-white/80 backdrop-blur-md flex items-center justify-center text-[#0f172a] hover:bg-[#0f172a] hover:text-white hover:scale-110 active:scale-95 transition-all duration-300 shadow-md cursor-pointer group lg:opacity-0 lg:group-hover/gallery:opacity-100"
      ...
      <div
        ...
        tabIndex={0}
        role="region"
        aria-label="Circular image gallery. Use left and right arrow keys or buttons to navigate."
      />

      {/* Right Chevron Button */}
      <button
        onClick={handleNext}
        className="absolute right-6 top-1/2 -translate-y-1/2 z-30 w-12 h-12 rounded-full border border-slate-200/50 bg-white/80 backdrop-blur-md flex items-center justify-center text-[#0f172a] hover:bg-[#0f172a] hover:text-white hover:scale-110 active:scale-95 transition-all duration-300 shadow-md cursor-pointer group lg:opacity-0 lg:group-hover/gallery:opacity-100"
